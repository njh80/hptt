...
#pragma omp parallel for
   for(uint32_t j=0; j < sizeOuter; ++j)
   {
      ...
      floatType* __restrict__ B_ = B + indexOffsetB + offsetB[0] + (j * outerSizeB[0]);

      if( beta == (floatType) 0 )
         for(int i=0; i < sizeInner; ++i) {
#ifdef DEBUG
            printf("A[%d] = %e -> B[%d] = %e\n", ((i + offsetA[perm[0]]) * strideAinner) + indexOffsetA, A_[(i + offsetA[perm[0]]) * strideAinner], i + offsetB[0] + indexOffsetB + (j * outerSizeB[0]), B_[i]);
#endif
            if( conjA )
               B_[i] = alpha * std::conj(A_[(i + offsetA[perm[0]]) * strideAinner]).real();
            else
               B_[i] = alpha * A_[(i + offsetA[perm[0]]) * strideAinner];}
      else
         for(int i=0; i < sizeInner; ++i) {
#ifdef DEBUG
            printf("A[%d] = %e -> B[%d] = %e\n", ((i + offsetA[perm[0]]) * strideAinner) + indexOffsetA, A_[(i + offsetA[perm[0]]) * strideAinner], i + offsetB[0] + indexOffsetB + (j * outerSizeB[0]), B_[i]);
#endif
            if( conjA )
               B_[i] = alpha * std::conj(A_[(i + offsetA[perm[0]]) * strideAinner]).real() + beta * B_[i];
            else
               B_[i] = alpha * A_[(i + offsetA[perm[0]]) * strideAinner] + beta * B_[i];}
   }
}

template void transpose_ref<float>( uint32_t *size, uint32_t *perm, int dim, 
      const float* __restrict__ A, float alpha, int *outerSizeA, int *offsetA, 
      float* __restrict__ B, float beta, int *outerSizeB, int *offsetB, const bool conjA);
template void transpose_ref<FloatComplex>( uint32_t *size, uint32_t *perm, int dim, 
      const FloatComplex* __restrict__ A, FloatComplex alpha, int *outerSizeA, int *offsetA, 
      FloatComplex* __restrict__ B, FloatComplex beta, int *outerSizeB, int *offsetB, const bool conjA);
template void transpose_ref<double>( uint32_t *size, uint32_t *perm, int dim, 
      const double* __restrict__ A, double alpha, int *outerSizeA, int *offsetA, 
      double* __restrict__ B, double beta, int *outerSizeB, int *offsetB, const bool conjA);
template void transpose_ref<DoubleComplex>( uint32_t *size, uint32_t *perm, int dim, 
      const DoubleComplex* __restrict__ A, DoubleComplex alpha, int *outerSizeA, int *offsetA, 
      DoubleComplex* __restrict__ B, DoubleComplex beta, int *outerSizeB, int *offsetB, const bool conjA);
